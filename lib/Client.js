...
Client.prototype.close = Client.prototype.end;
...
Client.prototype.abort = function abort(cb_) {
  var cb = (typeof cb_ === 'function' ? cb_ : null);
  if (!this._s || !this._s.writable) {
    cb && cb(new Error('Not connected'));
    return;
  }
  var key = this.key;
  if (!key) {
    cb && cb(new Error('Bad internal state, cannot abort'));
    return;
  }
  var req = this._cur;
  if (!req)
  ...
  var err;
  var tempClient = new Client(this._cfg);
  tempClient._p.on('ready', () => {
    if (this._cur === req)
      tempClient._p.cancel(key.pid, key.key);
    else
      tempClient._p.terminate();
  });
  tempClient.on('error', (err_) => {
    err = err_;
  });
  if (cb) {
    tempClient.on('close', () => {
